...
public class TdFMain {
    ...
    public static void main(String[] args) {
        ...
        // of median speeds. Now determine the median of this list.
        // Store the result in medianOfMedians.
        // Your code should go here.
        List<Double>allMedians = new ArrayList();
        for (String bikerName : allBikers.keySet()){

                if(allBikers.get(bikerName).getMedianSpeed() != 0) {
                    allMedians.add(allBikers.get(bikerName).getMedianSpeed());
        ...
        //System.out.println(allMedians);
        if(allMedians.size() % 2 == 1){
            medianOfMedians = allMedians.get(allMedians.size()/2);
        } else {
            medianOfMedians = (
                    allMedians.get(allMedians.size()/2  ) +
                            allMedians.get(allMedians.size()/2 -1 )
            )/2 ;
        }
